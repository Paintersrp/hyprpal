...
func (a *SidecarDockAction) Plan(ctx ActionContext) (layout.Plan, error) {
	...
	if override, ok := ctx.MonitorReservedOverrides[monitor.Name]; ok {
		reserved = override.Apply(reserved)
	}
	_, dock := layout.SplitSidecar(monitor.Rectangle, reserved, a.Side, a.WidthPercent, ctx.Gaps)
