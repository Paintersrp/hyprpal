...
Ask the daemon to compute the pending Hyprland dispatches. The optional `--explain` flag groups commands by rule, prints the predicate proof (including the values that satisfied each matcher), and enumerates the planned actions:

```bash
hsctl plan --explain
# rule: Coding:Dock comms on workspace 3
#   predicate:
#     {
#       "kind": "predicate",
#       "result": true,
...
The `--explain` flag (enabled by default) groups each dispatch by its `mode:rule` source—mirroring `hsctl plan --explain`—and prints the predicate proof tree (including the values evaluated) so you can understand why a rule matched or skipped.
...
- **Understand rule matches.** `rule.matched` entries list the mode, rule name, and reason string generated by predicates. When you need to see exactly why a predicate passed, add `--explain` to stream every matched rule (even at `--log-level=info`) along with its predicate proof tree—including the expected and actual values captured for each matcher. If a rule is skipped unexpectedly, bump to trace level and confirm whether the rule ever matches.
